import signature_module from "./@mootari.signature@553.js";

function _1(md, code, PINNED) {
  return (
    md`# Toolbox
...
function _9(signature, importAsset) {
  return (
...
function _importAsset(invalidation) {
  return (
...
function _11(signature, importImage, DOM) {
  return (
...
function _16(signature, scopedStyle) {
  return (
...
function _18(signature, getPinnedSlug) {
  return (
    signature(getPinnedSlug)
  )
}

function _getPinnedSlug(html) {
  return (
...
function _PINNED(getPinnedSlug) {
  return (
    getPinnedSlug()
  )
}
...
function _PINNED_LIB(getPinnedSlug) {
  return (
    getPinnedSlug({ name: '691ae3b95f02db79' })
  )
}

function _RUN_TESTS(deprecateImport, $0) {
  return (
...
function _deprecateImport(PINNED, PINNED_LIB) {
  return (
...
function _signature(deprecateImport, _dep_signature) {
  return (
    deprecateImport(_dep_signature, 'signature', false)
  )
}

function _code(deprecateImport, _dep_code) {
  return (
    deprecateImport(_dep_code, 'code', false)
  )
}

function _signature_theme(deprecateImport, _dep_signature_theme) {
  return (
    deprecateImport(_dep_signature_theme, 'signature_theme')
  )
}

function _createStepper(deprecateImport, _dep_createStepper) {
  return (
    deprecateImport(_dep_createStepper, 'createStepper')
  )
}

function _defaultTestRunner(deprecateImport, _dep_defaultTestRunner) {
  return (
    deprecateImport(_dep_defaultTestRunner, 'defaultTestRunner')
  )
}

function _defaultSignatureParser(deprecateImport, _dep_defaultSignatureParser) {
  return (
...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["signature", "fitSize"], _7);
  main.variable(observer("fitSize")).define("fitSize", _fitSize);
  main.variable(observer()).define(["signature", "importAsset"], _9);
  main.variable(observer("importAsset")).define("importAsset", ["invalidation"], _importAsset);
  main.variable(observer()).define(["signature", "importImage", "DOM"], _11);
  ...
  main.variable(observer()).define(["signature", "scopedStyle"], _16);
  main.variable(observer("scopedStyle")).define("scopedStyle", ["html"], _scopedStyle);
  main.variable(observer()).define(["signature", "getPinnedSlug"], _18);
  main.variable(observer("getPinnedSlug")).define("getPinnedSlug", ["html"], _getPinnedSlug);
  main.variable(observer()).define(["signature"], _20);
  ...
  const child1 = runtime.module(signature_module);
